...
for root, dirs, files in os.walk(original_dir, topdown=False):
    ...
    for original_filename in files:
        ...
        os.system(
            f'ffmpeg -i "{os.path.join(root, original_filename)}" -af silenceremove=1:0:-50dB -af "volume=5dB" -vn -ar '
            f'44100 -ac 2 -b:a 192k LOL.mp3')
        os.system(f'ffmpeg-normalize LOL.mp3 -f -c:a libmp3lame -o LOL.mp3')
        if os.path.exists(os.path.join(fixed_dir, new_file_name)):
            os.remove(os.path.join(fixed_dir, new_file_name))
        os.rename("LOL.mp3", os.path.join(fixed_dir, new_file_name))
...
with open(insert_into_file, "r+") as f:
    data = f.read()
    print(re.search(re.escape(insert_at_start) + ".*?" + re.escape(insert_at_end), data, flags=re.MULTILINE | re.DOTALL))
    data = re.sub(
